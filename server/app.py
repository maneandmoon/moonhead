...
class MoonPhaseResource(Resource):
    def get(self):
        ...
        params = {
            ...
            # "key": os.getenv("API_KEY"),  # Use environment variable for security
            "q": "Chicago",
        }
        ...
        for day in range(1, end_of_month + 1):
            params["dt"] = start_of_month.replace(day=day).strftime('%Y-%m-%d')
            ...
            try:
                ...
                if moon_phase and moonrise_time_str != "No moonrise":
                    moonrise_time = datetime.strptime(moonrise_time_str, '%I:%M %p')
                    full_moonrise_datetime = datetime.combine(start_of_month.replace(day=day).date(), moonrise_time.time())

                    moon_phase_data = {
                        'phase': moon_phase,
                        'date': full_moonrise_datetime.strftime('%Y-%m-%d %H:%M'),
